...
class TestDirectoryAuthority:
    ...
    def test_bridge_authority_default_false(self):
        """Test bridge_authority defaults to False."""
        auth = DirectoryAuthority(
            nickname="test",
            ip="192.0.2.1",
            dirport=9131,
        ...
        assert auth.bridge_authority is False


class TestDirectoryAuthorityList:
    """Tests for the hardcoded DIRECTORY_AUTHORITIES list."""

    def test_has_nine_authorities(self):
        """Test that we have exactly 9 directory authorities."""
        assert len(DIRECTORY_AUTHORITIES) == 9
    ...
    def test_all_authorities_have_unique_nicknames(self):
        ...
        nicknames = [auth.nickname for auth in DIRECTORY_AUTHORITIES]
    ...
    def test_all_authorities_have_unique_v3idents(self):
        ...
        v3idents = [auth.v3ident for auth in DIRECTORY_AUTHORITIES]
        assert len(v3idents) == len(set(v3idents))

    def test_known_authority_moria1(self):
        """Test moria1 authority has correct values."""
        moria1 = next(
            (auth for auth in DIRECTORY_AUTHORITIES if auth.nickname == "moria1"),
            None,
        )
        assert moria1 is not None
        assert moria1.ip == "128.31.0.34"
        assert moria1.dirport == 9131
        assert moria1.orport == 9101
        assert moria1.v3ident == "D586D18309DED4CD6D57C18FDB97EFA96D330566"

    def test_known_authority_tor26(self):
        """Test tor26 authority has correct values."""
        tor26 = next(
            (auth for auth in DIRECTORY_AUTHORITIES if auth.nickname == "tor26"),
            None,
        )
        assert tor26 is not None
        assert tor26.ip == "86.59.21.38"
        assert tor26.dirport == 80
        assert tor26.orport == 443

    def test_authorities_with_ipv6(self):
        """Test that some authorities have IPv6 addresses."""
        ipv6_authorities = [
            auth for auth in DIRECTORY_AUTHORITIES if auth.ipv6_address is not None
        ]
        ...
        gabelmoo = next(
            (auth for auth in DIRECTORY_AUTHORITIES if auth.nickname == "gabelmoo"),
            None,
        )
...
class TestGetRandomAuthority:
    ...
    def test_returns_valid_authority(self):
        ...
        assert auth is not None
        assert auth in DIRECTORY_AUTHORITIES
