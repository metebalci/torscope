...
class CircuitBuilder:
    ...
    async def build_circuit(
        ...
        """
        Build a circuit with real-time events.
        ...
        try:
            # Path selection
            await self.emit("path.selecting", {"hops": hops, "port": port})
            ...
            await self.emit(
                "path.selected",
                {
                    "hops": path.hops,
                    "routers": [
                        _router_info(r, role) for r, role in zip(routers, roles, strict=True)
                    ],
                },
            )
                ...
                for i, (router, ntor_key) in enumerate(zip(routers, ntor_keys, strict=True)):
                    ...
                    if i == 0:
                        extend_fn = functools.partial(
                            circuit.extend_to, router.fingerprint, ntor_key
                        )
                    else:
                        extend_fn = functools.partial(
                            circuit.extend_to,
